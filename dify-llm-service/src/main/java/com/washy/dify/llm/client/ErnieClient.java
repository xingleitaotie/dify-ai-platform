 ...
 * @author washy
 */
@Component("ernieClient")
@Slf4j
public class ErnieClient implements LlmClient {
    ...
    public ErnieClient(LlmProperties properties) {
        this.restTemplate = new RestTemplate();
    ...
    public String chat(List<ChatMessage> messages) {
        try {
            ...
            if (response == null || !response.containsKey("result")) {
                throw new LlmException("文心一言返回异常");
            }
            return response.get("result").toString();
        } catch (Exception e) {
            log.error("文心一言调用失败", e);
            throw new LlmException("文心一言调用失败：" + e.getMessage());
        }
    }
