...
@Slf4j
public class SparkClient implements LlmClient {
    ...
    public SparkClient(LlmProperties properties) {
        this.restTemplate = new RestTemplate();
    ...
    public String chat(List<ChatMessage> messages) {
        try {
            ...
            headerMap.put("app_id", llmProperties.getApiKey());

            Map<String, Object> chatMap = new HashMap<>();
            chatMap.put("temperature", llmProperties.getTemperature());
            chatMap.put("max_tokens", llmProperties.getMaxTokens());

            Map<String, Object> parameterMap = new HashMap<>();
            parameterMap.put("chat", chatMap);

            Map<String, Object> messageMap = new HashMap<>();
            messageMap.put("text", messages);

            Map<String, Object> payloadMap = new HashMap<>();
            payloadMap.put("message", messageMap);

            Map<String, Object> request = new HashMap<>();
            request.put("header", headerMap);
            request.put("parameter", parameterMap);
            ...
            return text.get("content").toString();
        } catch (Exception e) {
            log.error("讯飞星火调用失败", e);
            throw new LlmException("讯飞星火调用失败：" + e.getMessage());
        }
    }
    ...
    public String chat(String prompt, String history) {
        ...
        messages.add(ChatMessage.system("上下文：" + history + "\n"));
